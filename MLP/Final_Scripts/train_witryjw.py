...
''' Run 1:
params = {
    'layers' : [(100,200,100), (100,200,200,100), (100,100,200,100,100)],
...
params = [{'layers': (100,200,200,100),
    ...
    'layers': (100,100,200,100,100),
    'alpha': 5e-7,
    'batch_size': 32,
    'num_epochs': 600 #diff to additively train
}]
'''
...
### test code with params below; num_epochs = 1
'''
params = {
    'layers' : [(100,200,100),(200,300,200)],
    'alpha' : [0.1],
    'batch_size': [100],
    'num_epochs': [1]
}
...
model_params = []
t0 = time()
i = 18 # num models already tested
    ...
    for alpha in params['alpha']:
        for batch in params['batch_size']:
            for epoch in params['num_epochs']:
                model_params = {'layers': layer, 'alpha':alpha, 'batch_size': batch, 'num_epochs': epoch}
                modeli = MLP_Cells(num_epochs=epoch,batch_size=batch,alpha=alpha,layers=layer)
                X_test,y_test = modeli.fit()
                modeli.model.save('./models/mlp_witryjw'+str(i)+'.hdf5')
                f1 = f1_score(y_test,modeli.predict(X_test),average='macro')
                cohen = cohen_kappa_score(y_test,modeli.predict(X_test))
                print(f1,cohen)

                with open('./models/model'+str(i)+'.txt', 'a') as logfile:
                    ...
                    for key in model_params:
                        logfile.write(key + ': ' + str(model_params[key]))
                        logfile.write('\n')
                    logfile.write('--------')
                i += 1
...
### Model test over time

'''
for param in params:
    modeli = MLP_Cells(num_epochs=param['num_epochs'], alpha=param['alpha'], batch_size=param['batch_size'],layers=param['layers'],save_every=200)
    X_test, y_test = modeli.fit()
    #modeli.model.save('./models/mlp_witryjw' + str(i) + '.hdf5')
    f1 = f1_score(y_test, modeli.predict(X_test), average='macro')
    cohen = cohen_kappa_score(y_test, modeli.predict(X_test))
    ...
    with open('./models/model' + str(i) + '.txt', 'a') as logfile:
        ...
        logfile.write('\n')
        for key in param:
            logfile.write(key + ': ' + str(param[key]))
            logfile.write('\n')
        logfile.write('--------')
    i += 1
    t0 = time()
...
### retraining on outputs of previous models
...
for i in range(2,classes):
    modeli = MLP_Cells(num_epochs=params['num_epochs'], alpha=params['alpha'], batch_size=params['batch_size'],
                       layers=params['layers'], save_every=20,individual=i,num_classes=2)
    X_test,y_test = modeli.fit()
    #modeli.model.save('./models/mlp_witryjw' + str(i) + '.hdf5')
    f1 = f1_score(y_test, modeli.predict(X_test), average='macro')
    cohen = cohen_kappa_score(y_test, modeli.predict(X_test))
    ...
    with open('./models/model_ind' + str(i) + '.txt', 'a') as logfile:
        logfile.write('RUNTIME: ' + str(time() - t0))
        logfile.write('\n')
        logfile.write('f1 Score: ' + str(f1))
        logfile.write('\n')
...
# Use this to train on all model outputs
def initialize_model(dropout,layers,num_classes,input_dim,alpha):
    model = initialize_input_layer(layers,dropout,input_dim)
    model = initialize_middle_layers(model,dropout,layers)
    model = initialize_final_layer(model,num_classes)
    return compile_model(model,alpha)


def initialize_input_layer(layers,dropout,input_dim):
    ### build first keras layer; images are 101X100X3 so input dim =
    if type(dropout) == type(np.asarray([])) or type(dropout) == type([]):
        model = Sequential([
            Dense(layers[0], input_dim=input_dim, kernel_initializer=weight_init),
            Activation('relu'),
            Dropout(dropout[i]),
            BatchNormalization()
        ])
...
def initialize_middle_layers(model,dropout,layers):
    for i in range(1, len(layers)):
        model.add(Dense(layers[i], activation='relu', kernel_initializer=weight_init))
        if type(dropout) == type(np.asarray([])):
...
def initialize_final_layer(model,num_classes):
    model.add(Dense(num_classes, activation='softmax', kernel_initializer=weight_init))
    return model
...
X_train, X_test, y_train, y_test = modeli.read_data(datapath='../train/')
X_train, X_test, y_train, y_test = modeli.data_preparation(X_train, X_test, y_train, y_test)

#models = ['mlp_witryjw0.hdf5','mlp_witryjw1.hdf5','mlp_witryjw2.hdf5','mlp_witryjw3.hdf5','mlp_witryjw4.hdf5']
models = ['mlp_witryjw0.hdf5','mlp_witryjw1.hdf5','mlp_witryjw2.hdf5','mlp_witryjw3.hdf5','mlp_witryjw4.hdf5']
#X_train0 = load_model(models[0]).predict(X_train)

#X_test0 = load_model(models[0]).predict(X_test)
'''
for model in models[1:]:
    model_cur = load_model(model)
    X_train0 = np.hstack((X_train0, model_cur.predict(X_train)))
    X_test0 = np.hstack((X_test0, model_cur.predict(X_test)))
'''
y_pred = load_model(models[1]).predict(X_test)
y_pred = y_pred[:,1].reshape(-1,1)
for i in range(2,len(models)):
    model_cur = load_model(models[i])
    pred = model_cur.predict(X_test)
    #y_pred[:,i-1] *= pred[:,1]
    y_pred = np.hstack((y_pred,pred[:,1].reshape(-1,1)))

y_pred0 = load_model(models[0]).predict(X_test)
...
model_postfit = initialize_model(0.2,layers,4,12,1e-3)

checkpoints = keras.callbacks.ModelCheckpoint('mlp_witryjw_post{epoch:02d}-{val_loss:.2f}' + str(layers) + '.hdf5',
                                              monitor='val_loss', verbose=1, save_weights_only=False,
                                              period=10)

model_postfit.fit(X_train0,y_train,epochs=50,validation_data=(X_test0, y_test), verbose=1, callbacks=[checkpoints])
y_test = np.argmax(y_test,axis=1)
y_pred = np.argmax(model_postfit.predict(X_test0),axis = 1)
f1 = f1_score(y_test, y_pred, average='macro')
cohen = cohen_kappa_score(y_test, y_pred)
print(f1, cohen)

with open('./models_postfit/model.txt', 'a') as logfile:
    logfile.write('RUNTIME: ' + str(time() - t0))
    logfile.write('\n')
    logfile.write('f1 Score: ' + str(f1))
    logfile.write('\n')
